	...
	<head>
		...
		<style>
			*{margin:0; padding:0;}
			...
			.buttons_two{margin-top:20px; margin-bottom:30px; font-size:12pt; font-weight:bold; background:#7c27c7;
							border:none; width:120px; height:60px; color:white; outline:none;}
			.buttons_two:hover{color:#95e2f3;}
		</style>
		
		<script src="${pageContext.request.contextPath}/resources/js/httpRequest.js"></script>
		<script>
			function reply() {
				location.href = "i_reply_form.do?id=${imdaein_vo.id}&idx=${vo.idx}&page=${param.page}";
			}
		</script>
	</head>
	<body>
		...
		<form name="f" method="post">
			...
			<table width="1100" align="center">
				...
				<tr>
					<td align="center">
						<%-- 목록으로 돌아가기 --%>
						<input type="button" class="buttons_two" value="목록으로" onclick="location.href='i_login_main.do?id=${imdaein_vo.id}&page=${param.page}'">
						
						<c:if test="${vo.depth lt 1}">
							<!-- 답글 등록 -->
							<input type="button" class="buttons_two" value="답글 등록" onclick="reply();">
						</c:if>
					</td>	
				</tr>
			</table>
