...
<html>
	...
	<body>
		...
		<table width="600" height="300" align="left">
						...
						<tr class="tr_a_1">
							...
							<td class="td_b_1 left">
								...
								<c:if test="${vo.del_info ne -1}">
									...
									<!-- 로그인하지 않았을 경우 -->
									<c:if test="${empty sessionScope.manage_vo and empty sessionScope.gongin_vo and empty sessionScope.imdaein_vo}">
										<a href="view.do?idx=${vo.idx}&page=${empty param.page ? 1 : param.page}" class="num">${vo.subject}</a>
									</c:if>
									...
									<!-- 공인중개사가 로그인한 경우 -->
									<c:if test="${empty sessionScope.manage_vo and not empty sessionScope.gongin_vo and empty sessionScope.imdaein_vo}">
										<a href="g_view.do?idx=${vo.idx}&page=${empty param.page ? 1 : param.page}" class="num">${vo.subject}</a>
									</c:if>
											
									<!-- 임대인이 로그인한 경우 -->
									<c:if test="${empty sessionScope.manage_vo and empty sessionScope.gongin_vo and not empty sessionScope.imdaein_vo}">
										<a href="i_view.do?idx=${vo.idx}&page=${empty param.page ? 1 : param.page}" class="num">${vo.subject}</a>
									</c:if>
								</c:if>
			...
			<!-- 관리사가 아닌 경우 -->
			<c:if test="${(empty sessionScope.manage_vo and empty sessionScope.gongin_vo and empty sessionScope.imdaein_vo)
							 || (empty sessionScope.manage_vo and not empty sessionScope.gongin_vo and empty sessionScope.imdaein_vo)
							 || (empty sessionScope.manage_vo and not empty sessionScope.gongin_vo and empty sessionScope.imdaein_vo)}">
				<tr>
					<td height="5"></td>
				</tr>
